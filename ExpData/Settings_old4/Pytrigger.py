...
class ElectricShock(Task):
    ...
    def Shock(self, start):
        ...
        if self.shockDuration > 0:
            ...
            self.WriteAnalogF64(len(self.stim), True, 10.0, DAQmx_Val_GroupByChannel, self.stim, byref(self.sampsWritten), None)
...
class SocketConnection:
    ...
    def connect(self):
        ...
        try:
            while True:
                ...
                self.read_from_client(clientsocket, clientaddress)
                # t = threading.Thread(target = read_from_client, args = (clientsocket, clientaddress))
                # t.start()
            clientsocket.close()
            print('No input.')
        except IOError as e:
    ...
    def read_from_client(self, clientsocket, clientaddress):
        content = None
        try:
